...
jQuery(function ($) {
  ...
  $("#btn_contact_submit").click(function () {
    //get input field values
    var user_name =
      $("input[name=f_name]").val() + " " + $("input[name=l_name]").val();
    var user_email = $("input[name=email]").val();
    var user_telephone = $("input[name=phone]").val();
    var user_message = $("textarea[name=message]").val();
      ...
      $('form.newsletter input[type="submit"]').bind("click", function (event) {
        ...
        if (validateEmail()) {
          ...
          $("#result1")
            .hide()
            .html(
              "<div class='alert-danger'>Please provide valid email address.</div>"
            )
            .slideDown(200);
        }
      });
...
function register($form) {
  $.ajax({
    ...
    success: function (data) {
      var resultMessage =
        data.msg || "<div class='alert-danger'>Please try again later.</div>";

      if (data.result != "success") {
        if (data.msg && data.msg.indexOf("already subscribed") >= 0) {
          resultMessage =
            "<div class='alert-danger'>You're already subscribed.</div>";
          $("#result1").hide().html(resultMessage).slideDown(200);
        } else {
          resultMessage =
            "<div class='alert-danger'>Please try again later.</div>";
          $("#result1").hide().html(resultMessage).slideDown(200);
        }
      } else {
        resultMessage =
          "<div class='alert-success'>Thank you! Please check your email.</div>";
        $("#result1").hide().html(resultMessage).slideDown(200);
