...
$c_title = $row_edit['title'];



$c_image = $row_edit['image'];

$new_c_image = $row_edit['image'];
$c_featured = $row_edit['featured'];
$c_active = $row_edit['active'];
...
<input type="radio" name="cat_featured" value="yes" 
<?php if($c_featured == 'no'){}else{ echo "checked='checked'"; } ?>>

<label>Yes</label>

<input type="radio" name="cat_featured" value="no" 
<?php if($c_featured == 'no'){ echo "checked='checked'"; }else{} ?>>
...
<input type="radio" name="cat_active" value="yes" 
<?php if($c_active == 'no'){}else{ echo "checked='checked'"; } ?>>

<label>Yes</label>

<input type="radio" name="cat_active" value="no" 
<?php if($c_active == 'no'){ echo "checked='checked'"; }else{} ?>>
...
if(isset($_FILES['cat_image']['name']) && !empty($cat_image))
                {
                    // $cat_image = $_FILES['cat_image']['name'];
                  $ext = end(explode('.', $cat_image));
                  $cat_image = "Cat-Name-" . rand(0000, 9999) . '.' . $ext;
                  $temp_name = $_FILES['cat_image']['tmp_name'];
                  $upload=move_uploaded_file($temp_name,"other_images/$cat_image");
                  if($upload==false){
...
$update_cat = "update category set title='$cat_title',image='$cat_image',featured='$cat_featured',active='$cat_active' where id='$c_id'";
